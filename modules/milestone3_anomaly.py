...
def compute_anomaly_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # We add every single rule as a point
    flag_cols = [
        "rule_tachycardia", "rule_bradycardia", "rule_low_spo2", 
        "rule_sleep_steps", "rule_sleep_hr", "prophet_anomaly", "dbscan_anomaly"
    ]
    
    # Ensure columns exist even if they are 0
    for col in flag_cols:
        if col not in df.columns:
            df[col] = 0
            
    # SUM THEM UP: If 5 rules are broken, score is 5.
    df["anomaly_score"] = df[flag_cols].sum(axis=1)
    
    # Define severity based on the new cumulative score
    df["severity"] = "normal"
    df.loc[df["anomaly_score"] >= 1, "severity"] = "low"
    df.loc[df["anomaly_score"] >= 3, "severity"] = "medium"
    df.loc[df["anomaly_score"] >= 5, "severity"] = "high"
